...
[`notebooks/demo_cache.json`](https://github.com/Abivarma/smart-llm-fraud-detection/blob/main/notebooks/demo_cache.json).

---

## Aggregate Summary

| Metric | Naive (Total) | Pipeline (Total) | Savings |
|--------|--------------|-----------------|---------|
...
| # | Scenario | Txns | Fraud | Naive Tokens | Pipeline Tokens | Savings | Naive Acc | Pipeline Acc |
|---|----------|------|-------|-------------|----------------|---------|-----------|-------------|
...
Scenario 6 is notable: the pipeline used **zero tokens** because no filters triggered
...
<p><strong>Why the pipeline wins</strong>: Code detects time_diff &lt; 300s deterministically; Naive sends 22K tokens for the same result.</p>
...
<p><strong>Pipeline trajectory</strong>:</p>
<ul>
...
<table>
<tr><th>Metric</th><th>Naive</th><th>Pipeline</th></tr>
...
<p><strong>Why the pipeline wins</strong>: Code checks location+time pairs; RAG cannot do distance/time math.</p>
...
<p><strong>Why the pipeline wins</strong>: Code computes leave-one-out std dev (z=170.61); LLM confirms category shift.</p>
...
<p><strong>Why the pipeline wins</strong>: Multi-filter cross-check: device_shift catches subtle ATO pattern.</p>
...
<p><strong>Why the pipeline wins</strong>: Velocity filter catches count + timing; RAG finds no similar historical cases for $1 transactions.</p>
...
<p><strong>Why the pipeline wins</strong>: Filters correctly pass through (no anomaly). Naive over-flags due to high amounts.</p>
...
<p>This is the most dramatic example: the pipeline achieved perfect accuracy with zero cost, while Naive spent $0.0035 and still got it wrong.</p>
</details>

<details>
<summary><strong>Scenario 7: Mixed Batch</strong> -- 5 users, 15 txns, 2 fraudulent users</summary>

<p><strong>Why the pipeline wins</strong>: Per-user sub-calls analyze 5 users independently; Naive sends all 15 + 500 cases in one prompt.</p>
...
<p><strong>Why the pipeline wins</strong>: Geo + velocity filters both trigger = high confidence; RAG misses temporal pattern.</p>
...
| Scenario | Naive Error | Type | Pipeline Result |
|----------|------------|------|----------------|
...
Per-transaction cost: Naive = $0.000559, Pipeline = $0.000016.
...
- **Cached results**: All API responses saved in `demo_cache.json` for reproducibility
- **Naive baseline**: Sends all transactions + 500 historical fraud cases in a single prompt
- **Pipeline approach**: 4-phase filter-then-verify with deterministic filters + targeted LLM sub-calls
